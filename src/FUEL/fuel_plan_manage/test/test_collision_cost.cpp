...
#include <fuel_bspline_opt/bspline_optimizer.h>
#include <fuel_bspline/non_uniform_bspline.h>
#include <fuel_traj_utils/planning_visualization.h>
...
int main(int argc, char** argv) {
  ...
  BsplineOptimizer::Ptr fuel_bspline_opt;
  fuel_bspline_opt.reset(new BsplineOptimizer);
  fuel_bspline_opt->setParam(node);
  fuel_bspline_opt->setEnvironment(env);
  ...
  /* ---------- optimization ---------- */
  fuel_bspline_opt->setControlPoints(ctrl_pts);
  fuel_bspline_opt->setKnotSpan(dt);

  t1 = ros::Time::now();

  fuel_bspline_opt->optimize(BsplineOptimizer::HARD_CONSTRAINT, false);

  t2 = ros::Time::now();
  cout << "optimization time: " << (t2 - t1).toSec() << endl;

  ctrl_pts = fuel_bspline_opt->getControlPoints();
