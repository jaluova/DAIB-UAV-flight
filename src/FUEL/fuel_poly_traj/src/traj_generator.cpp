...
int main(int argc, char** argv) {
  ...
  fuel_poly_traj.init();
  vector<Eigen::Vector3d> traj_vis = fuel_poly_traj.getTraj();
  ...
  while (ros::ok()) {
    ...
    Eigen::Vector3d pt = fuel_poly_traj.evaluate(tn);
    Eigen::Vector3d vel = fuel_poly_traj.evaluateVel(tn);
    Eigen::Vector3d acc = fuel_poly_traj.evaluateAcc(tn);
